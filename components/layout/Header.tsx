...
import { Menu, Search, ShoppingCart, X, ChevronDown, Tag } from "lucide-react";
...
export default function Header() {
  ...
  return (
    <>
      ...
      <div className="bg-stone-900 text-stone-200 text-xs text-center py-2 tracking-widest uppercase font-light">
      ...
      >
              ...
              {navItems.map(({ href, label, mega, isSale }) => (
                ...
                >
                  ...
                  >
                    ...
                    {isSale && (
                      <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] font-bold rounded-full px-1 leading-4">
                        HOT
                      </span>
                    )}
                  </Link>
            ...
            <div className="lg:hidden mt-3 animate-in slide-in-from-top duration-200">
              <form className="relative">
                <input
                  ...
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-10 pr-4 py-2.5 text-sm border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-stone-800 focus:border-transparent bg-stone-50"
                  aria-label="Search shoes"
                  autoFocus
                />
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-stone-400" />
              ...
              <div className="flex flex-col pb-4 border-b border-stone-200 space-y-1">
                ...
                {menCategories.map(({ href, label }) => (
                  <Link
                    ...
                    className="text-sm py-1.5 px-3 rounded-md text-stone-700 hover:text-stone-900 hover:bg-stone-100 transition-all"
                  >
                    {label}
                  </Link>
                ))}
        ...
        {activeMega && (
          <div
            className="hidden md:block absolute left-0 right-0 bg-white border-b border-stone-200 shadow-xl z-40 animate-in fade-in slide-in-from-top-2 duration-150"
          >
                  ...
                  <div className="grid grid-cols-2 gap-x-10 gap-y-2">
                    {(activeMega === "men" ? menCategories : womenCategories).map(({ href, label }) => (
                      <Link
                        key={href}
                        href={href}
                        className="text-sm text-stone-700 hover:text-stone-900 font-medium py-1 hover:underline underline-offset-4 transition-all"
                      >
                        {label}
                      </Link>
                    ))}
